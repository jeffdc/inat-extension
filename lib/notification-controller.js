...
class NotificationController {
  ...
  async loadCachedMentions() {
    try {
      const cachedMentions = await Notifications.MentionCache.load();
      this.store.add(cachedMentions);
      return cachedMentions;
    } catch (err) {
      console.warn('[iNat Ext] Failed to load cached mentions:', err);
      return [];
    }
  }

  async persistMentions() {
    try {
      await Notifications.MentionCache.save(this.store.getByCategory('mention'));
    } catch (err) {
      console.warn('[iNat Ext] Failed to persist mentions:', err);
    }
  }
}
