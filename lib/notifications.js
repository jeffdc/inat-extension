...
function serializeNotification(notification) {
  ...
  return {
    ...
    category: notification.category || 'identification',
    viewed: notification.viewed ?? null,
    createdAt: notification.createdAt instanceof Date
      ? notification.createdAt.toISOString()
      : (notification.createdAt || null),
    observationId: notification.observationId || '',
    observationUrl: notification.observationUrl || '',
    user: {
      login: notification.user?.login || 'unknown',
      name: notification.user?.name || notification.user?.login || 'Unknown',
      iconUrl: notification.user?.iconUrl || null
    },
    body: notification.body || null,
    taxon: notification.taxon ? {
      id: notification.taxon.id || null,
      name: notification.taxon.name || null,
      commonName: notification.taxon.commonName || null
    } : null,
    observation: notification.observation ? {
      taxon: notification.observation.taxon ? {
        id: notification.observation.taxon.id || null,
        name: notification.observation.taxon.name || null,
        commonName: notification.observation.taxon.commonName || null
      } : null,
      thumbnail: notification.observation.thumbnail || null,
      mediumPhoto: notification.observation.mediumPhoto || null,
      observer: notification.observation.observer || null,
      observedOn: notification.observation.observedOn || null,
      placeGuess: notification.observation.placeGuess || null,
      qualityGrade: notification.observation.qualityGrade || null,
      identificationsCount: notification.observation.identificationsCount || 0
    } : null,
...
const MentionCache = {
  STORAGE_KEY: 'inat_cached_mentions_v1',

  async load() {
    const result = await browser.storage.local.get(this.STORAGE_KEY);
    const cached = Array.isArray(result[this.STORAGE_KEY]) ? result[this.STORAGE_KEY] : [];

    return cached
      .map(item => createNotification(item))
      .filter(item => item.category === 'mention');
  },

  async save(mentions) {
    const serialized = (mentions || [])
      .filter(item => item?.category === 'mention')
      .map(item => serializeNotification(item))
      .filter(Boolean);

    await browser.storage.local.set({ [this.STORAGE_KEY]: serialized });
